"""
Fetch 5-minute marginal emissions rates (MER) for CAISO DLAPs from the SGIP Signal API
and aggregate to hourly MER.

API: SGIP GHG Signal (https://sgipsignal.com)
Regions: SGIP_CAISO_PGE, SGIP_CAISO_SCE, SGIP_CAISO_SDGE
Output: dlap_mer_{tag}

Usage:
    python fetch_dlap_mer.py --start 2025-01-01 --end 2026-01-01 --tag 2025
"""
...
with open(CREDS_PATH) as _f:
    creds = json.load(_f)
    API_USERNAME = creds["sgip_username"]
    API_PASSWORD = creds["sgip_password"]
...
def main():
    ...
    sql_table_name = "dlap_mer" + "_" + args.tag

    start_date = datetime.strptime(args.start, "%Y-%m-%d").replace(tzinfo=timezone.utc)
    end_date = datetime.strptime(args.end, "%Y-%m-%d").replace(tzinfo=timezone.utc)

    records = fetch_all(API_USERNAME, API_PASSWORD, start_date, end_date)
